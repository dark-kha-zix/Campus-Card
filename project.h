...
public:
	...
	~Student();						//退学处理
	bool recharge(int amount);		//充值成功，卡没有挂失时，即充值金额大于0小于等于500返回true，否则返回false
	bool expense(int amount);		//卡没有挂失时，消费金额大于0小于等于500并且小于当前余额返回true，否则返回false
...
public:
	...
	vector<int> get_charge_record();	//返回充值记录
	vector<int> get_consume_record();	//返回消费记录

private:
	Student* _stu;						//所属学生
	int _id, _remain;					//卡号，余额
	vector <int> _charge, _consume;		//充值，消费记录	
	bool _flag;							//是否为挂失
